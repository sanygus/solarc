...
$fvoff="/home/pi/python/voff.var";
$fvon="/home/pi/python/von.var";
...
if(file_exists($fosv)){
  ...
  $light_sensor=round($light_sensor);
}else{$light_sensor="Not found";};

if(file_exists($fvlt)){
  $voltage_sensor=file_get_contents($fvlt);
  $voltage_sensor=round($voltage_sensor,2);
}else{$voltage_sensor="Not found";};

if(file_exists($fvoff)){
  if($_POST['vch_ok']=="ok"){
    $current_float=floatval($_POST['voffval']);
    if(($current_float>0)and($current_float<5))
    {
      if(!file_put_contents($fvoff,$current_float))
      {
        $mess .= "Файл $fvoff не записан";
      };
    }else{$mess .= "Off V должно быть строго от 0 до 5";};
    unset($current_float);
  };
  $voltage_off=file_get_contents($fvoff);
}else{$voltage_off="Not found";};

if(file_exists($fvon)){
  if($_POST['vch_ok']=="ok"){
    $current_float=floatval($_POST['vonval']);
    if(($current_float>0)and($current_float<5))
    {
      if(!file_put_contents($fvon,$current_float))
      {
        $mess .= "<br>Файл $fvon не записан";
      };
    }else{$mess .= "On V должно быть строго от 0 до 5";};
    unset($current_float);
  };
  $voltage_on=file_get_contents($fvon);
}else{$voltage_on="Not found";};
  ...
  <body>
      <center><br><div class="mdl-spinner mdl-js-spinner" id="loadspinner"></div><br><br><br>
        <form action="/" method="POST">
          ...
          <button type=submit name=act value=off onclick="document.getElementById('loadspinner').className += ' is-active';" class="mdl-button mdl-js-button"<?php btst("off"); ?> disabled>Off</button>
        </form><br><br><br><br>
...
<form action="/" method="POST">
  <b>Power:</b>&emsp;&emsp;
  <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label" style="width:35px;"><input type="text" name="voffval" class="mdl-textfield__input" id="textfield1" value="<?php echo($voltage_off); ?>"><label class="mdl-textfield__label" for="textfield1">Off <</label></div>&emsp;&emsp;
  <div class="mdl-textfield mdl-js-textfield mdl-textfield--floating-label" style="width:35px;"><input type="text" name="vonval" class="mdl-textfield__input" id="textfield2" value="<?php echo($voltage_on); ?>"><label class="mdl-textfield__label" for="textfield2">On >=</label></div>&emsp;&emsp;
  <button type="submit" name="vch_ok" value="ok" class="mdl-button mdl-js-button">OK</button>
</form>
